...
const ProductReviews = ({ productId }) => {
  ...
  const handleSubmitReview = () => {
    ...
    const dateString = reviewDate.toLocaleString('en-US',{weekday: 'long', year: 'numeric', month: 'long',day: 'numeric',
        ...
        ? reviews.map(review =>
            review.id === editReviewID ? { ...review, text: newReview, rating, username, date: dateString } : review
          )
        : [...reviews, { id: Date.now(), text: newReview, rating, username, date: dateString }];
  ...
  const handleDelete = (reviewIdAayi)=>{
    const reviewEditWala = reviews.filter(review => review.id != reviewIdAayi)
    setReviews(reviewEditWala)
    localStorage.setItem(`reviews-${productId}`,JSON.stringify(reviewEditWala))
  ...
  return (
    ...
    <div id='kuch4' className='w-[30vw]  bg-white border-2 border-gray-300 p-6  gap-2 mx-auto shadow-lg'>
  ...
  <div className='mt-8 '>
    {reviews.length > 0 ? (
      reviews.map((review) => (
        <div key={review.id} className="border-b-2 bg-[#eeeeee] p-4 border-gray-200 py-6  shadow-lg">
          <h1 className="text-gray-800 font-bold"><span className='font-normal text-gray-500'>Name:</span> {review.username}</h1>
          <p className="text-gray-800 gont-bolf"> <span className='font-normal text-gray-500'>Review:</span> {review.text}</p>
          <span className="text-yellow-500"> <span  className='font-normal text-gray-500'>Rating:</span> {review.rating} ⭐</span>
          <p className='text-sm text-black font-medium '><span className='font-normal text-gray-500'>Submitted on:</span> {review.date}</p>
          <div className='flex gap-3 mt-2'>
          <button className='text-blue-600' onClick={()=>{handleEdit(review.id)}}>Edit Review</button>
          <button className='text-red-600' onClick={()=>{handleDelete(review.id)}}>Delete</button>
          </div>
        </div>
      ))
    ) : (
...
{addedMessage ? (
    <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 bg-green-600 text-white py-2 px-4 rounded">{addedMessage}</div>
) : null}
